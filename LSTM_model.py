...
def getProductIndicesAndPrices(df, T_x, styleToIndex):
    ...
    Y = []
    numBuckets = 89
    for i in range(0, len(df['text'])):#len(df['item_description'])):
        if (pd.isnull(df['text'][i]) == False): # Checks for Nan descriptions
            X.append((getIndexArrForSentence(df['text'][i], T_x)) )
        else:
